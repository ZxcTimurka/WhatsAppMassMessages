...
# Путь к вашему JSON-файлу ключа сервисного аккаунта Google
SERVICE_ACCOUNT_FILE = 'service_acc.json'

# ID вашей Google Таблицы (можно скопировать из URL таблицы)
SPREADSHEET_ID = '1MrpbtdzX6jrl98xvMQlA9Gg3S5dsffp16ni_K_ysO2w'

# Диапазон, содержащий номера телефонов
RANGE_NAME = 'Sheet1!B2:B'
...
def format_phone_number(raw_number):
    if not isinstance(raw_number, str):
        print(f"  Предупреждение: Получено не строковое значение: {raw_number}. Пропускаем.")
        return None
    ...
    if len(digits) == 11:
        if digits.startswith('8'):
            ...
            print(f"  Предупреждение: Номер '{raw_number}' -> '{digits}' имеет 11 цифр, но начинается не с 7 или 8. Пропускаем.")
            return None
    elif len(digits) == 10:
         # Если 10 цифр, скорее всего, отсутствует код страны, добавляем '7'
         print(f"  Предупреждение: Номер '{raw_number}' -> '{digits}' имеет 10 цифр. Добавляем '7' в начало.")
         return '7' + digits
    else:
        # Неверная длина
        print(f"  Предупреждение: Номер '{raw_number}' -> '{digits}' имеет неверную длину ({len(digits)}). Пропускаем.")
        return None


def get_phone_numbers_from_sheet(spreadsheet_id, range_name):
    if not os.path.exists(SERVICE_ACCOUNT_FILE):
        print(f"Ошибка: Файл сервисного аккаунта не найден по пути: {SERVICE_ACCOUNT_FILE}")
        return None
    ...
    try:
        ...
        if not values:
            ...
            print(f"Найдено строк в таблице: {len(values)}. Начинаем обработку и форматирование:")
            ...
            print("-" * 20)
            print(f"Итоговый список номеров для отправки ({len(formatted_numbers)} шт.):")
        ...
        print(f"Произошла непредвиденная ошибка при чтении или форматировании номеров: {e}")
        return None
...
def send_message(api_url, phone_number, message):
    ...
    try:
        ...
        if e.response is not None:
             try:
                 print(f"  Ответ сервера: {e.response.text.encode('latin1').decode('utf8')}")
             except UnicodeDecodeError:
                 try:
                     print(f"  Ответ сервера: {e.response.text.encode('utf-8').decode('utf-8')}")
                 except Exception:
                     print(f"  Не удалось декодировать ответ сервера: {e.response.text}")
        return False
    except Exception as e:
...
def main():
    ...
    print(f"Всего номеров в таблице обработано (валидных): {total_numbers_to_send}")
...
if __name__ == '__main__':
    if API_URL == "TOKEN" or API_URL == "":
         ...
         print(f"Ошибка: Не указан или не найден файл SERVICE_ACCOUNT_FILE: {SERVICE_ACCOUNT_FILE}")
